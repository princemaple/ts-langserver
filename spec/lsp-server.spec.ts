...
import * as lspcalls from '../src/lsp-protocol.calls.proposed';
import {LspServer} from '../src/lsp-server';
import {TSCompletionItem} from '../src/completion';
...
`,
    ...
    await server.requestDiagnostics();
    await new Promise(resolve => setTimeout(resolve, 200));
    const diagnosticsForThisTest = diagnostics.filter(
      d => d!.uri === doc.uri || d!.uri === doc2.uri,
    );
    await new Promise(resolve => setTimeout(resolve, 200));
...
describe('formatting', () => {
  ...
  it('full document formatting', async () => {
    ...
    const result = lsp.TextDocument.applyEdits(
...
describe('signatureHelp', () => {
  it('simple test', async () => {
    ...
    assert.equal(
      'bar: string',
      result.signatures[result.activeSignature!].parameters![result.activeParameter!].label,
    );
...
describe('calls', () => {
  function resultToString(callsResult: lspcalls.CallsResult, direction: lspcalls.CallDirection) {
    ...
    const symbolToString = (symbol: lspcalls.DefinitionSymbol) =>
      `${symbol.name} (${symbol.location.uri.split('/').pop()}#${
    ...
    for (const call of callsResult.calls) {
      out.push(
        `  ${arrow} ${symbolToString(call.symbol)} - ${call.location.uri.split('/').pop()}#${
