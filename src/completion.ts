...
export function asCompletionItem(
  entry: import('typescript/lib/protocol').CompletionEntry,
  file: string,
  ...
  const item: TSCompletionItem = {
    ...
    data: {
      ...
      entryNames: [entry.source ? {name: entry.name, source: entry.source} : entry.name],
    },
  };
  ...
  if (replacementRange && replacementRange.start.line !== replacementRange.end.line) {
    replacementRange = lsp.Range.create(
      replacementRange.start,
      document.getLineEnd(replacementRange.start.line),
    );
  }
  if (insertText && replacementRange && insertText[0] === '[') {
    // o.x -> o['x']
